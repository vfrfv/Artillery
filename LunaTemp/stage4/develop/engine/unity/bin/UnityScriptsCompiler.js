...
if ( TRACE ) { TRACE( "Howitzer.PlayerShooting#Shoot", this ); }
                ...
                if (!this._firstShot) {
                    ...
                    this._firstShot = true; // Теперь следующие выстрелы пойдут как обычно
                } else {
                    if (this._isZoomed) {
                        this._timeController.StartSlowMotion();
...
if ( TRACE ) { TRACE( "Howitzer.Projectile#OnCollisionEnter", this ); }
                ...
                if (collision.gameObject.TryGetComponent$1(BehaviourAI.TankAI, hitTankAI)) {
                    hitTankAI.v.DisableTank();
                    hitTankAI.v.GetComponent(UnityEngine.BoxCollider).enabled = false;
                ...
                try {
                    while ($t.moveNext()) {
                        ...
                        if (UnityEngine.MonoBehaviour.op_Inequality(tankAI, null) && tankAI.IsAlive && UnityEngine.MonoBehaviour.op_Inequality(tankAI, hitTankAI.v)) {
                            tankAI.ShowSign();
                        }
                    }
                } finally {
